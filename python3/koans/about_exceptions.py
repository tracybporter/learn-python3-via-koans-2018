...
class AboutExceptions(Koan):
    ...
    def test_try_clause(self):
        ...
        self.assertEquals(AssertionError, ex2.__class__)
        self.assertEqual(False, isinstance(ex2, RuntimeError))

        # But ex2 is not a RuntimeError so not sure why we care here.
        self.assertTrue(issubclass(RuntimeError, Exception), "RuntimeError is a subclass of Exception")

        self.assertFalse(issubclass(AssertionError, RuntimeError), "AssertionError is NOT a subclass of RuntimeError")

        self.assertEqual(1, ex2.args.__len__())
