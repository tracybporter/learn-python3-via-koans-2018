...
class AboutIteration(Koan):
    ...
    def test_map_transforms_elements_of_a_list(self):
        ...
        mapping = map(self.add_ten, aList)
    ...
    def test_filter_returns_all_items_matching_criterion(self):
        ...
        names = ["Jim", "Bill", "Clarence", "Doug", "Eli", "AnotherBigName"]
        iterator = filter(is_big_name, names)

        self.assertEqual('Clarence', next(iterator))
        self.assertEqual('AnotherBigName', next(iterator))
    ...
    def test_reduce_will_blow_your_mind(self):
        ...
        result = functools.reduce(self.add, [2, 3, 4], 10)
        self.assertEqual(10 + 2 + 3 + 4, result)
        ...
        # Operates on an in iterable as defined by the function and reduces it to a single value which is stored in the accumulator.
        # The user provided function must adhere to the contract of (self, accumulator, item)
    ...
    def test_use_pass_for_iterations_with_no_body(self):
        ...
        self.assertEqual(4, num)  # right - because the last value in a range(m,n) is n-1
